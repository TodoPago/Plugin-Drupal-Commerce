...
class ControlFraudeService extends ControlFraude {
	...
	protected function completeCSVertical(){
		...
		if(property_exists($item,"csmdd28"))
			$datosCS["CSMDD28"] = $item->csmdd28[LANGUAGE_NONE][0]["value"];
		if(property_exists($item,"csmdd29"))
			$datosCS["CSMDD29"] = $item->csmdd29[LANGUAGE_NONE][0]["value"];
